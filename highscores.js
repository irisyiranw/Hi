/* Kitchen Rush high scores — saved only in this browser/device. */
'use strict';

(() => {
  const STORAGE_KEY = 'kitchen-rush-high-scores';
  const LIMIT = 5;
  ...
  function renderHighScores() {
    ...
    document.querySelectorAll('.high-scores-list').forEach(list => {
      if (!scores.length) {
        list.innerHTML = '<li class="high-scores-empty">No scores yet — play a game to be the first chef on the board!</li>';
        return;
      }
